<!DOCTYPE html>
<html lang="pt-br">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Academium+</title>
    <link rel="stylesheet" href="Styles\Home.css">
    <link rel="shortcut icon" href=".\Src\icon Academium.ico">
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css">
</head>
<body>
    <div id="Container">
        <div class="navbar">
            <div class="headerDivLogo">
                <a class="HeaderLogoText" href="./HomePage.php">
                    <img src=".\Src\Novo Logo Academium Transparente - Off White.png" class="HeaderLogo" width="40">
                    <span class="HeaderLogoTextSpan">Academium</span>
                    <sup class="HeaderLogoTextPlus">+</sup>
                </a>
            </div>
            <div class="headerDivButton">
                <a href="./LoginPage.php">
                    <button id="LoginButton"> Entrar </button> 
                </a> 
            </div>
        </div>
        <div class="Content">
            <div class="SearchArea">
                <p id="TextoTitu">Aqui você tem acesso a educação A+</p>
                <p id="TextoAuxilio">Procure os cursos que farão o seu currículo ter um upgrade</p>
                <div class="SearchAreaDiv">
                    <img src=".\Src\lupa.png" id="LupaEmoji">
                    <input type="text" placeholder="Oque você quer procurar?" id="InputAreaCourses">
                    <button id="BuscarCourse">Buscar</button>
                </div>
            </div>
        </div>
    </div>
    <div class="ContentText">
        <div class="ContentTextText">
            <p id="txtTitu">Cursos de todos os tipos</h2>
            <p id="txtComplementar">
                Aqui no Academium, você tem acesso a qualquer curso que precisar, de todas as áreas que puder
                imaginar.<br><br>Se precisa achar conhecimento, venha ver or cursos disponíveis no nosso site
            </p>
            <a href="./CursosPage.php"> <button id="buttonPageCourses">Vem de Academium</button> </a>
        </div>
        <img id="imgLingaguens" src=".\Src\lingaguens.png" alt="Logos de lingaguens">
    </div>

    <footer>
        <div class="footer-content">
            <a class="HeaderLogoText" href="">
                <span class="HeaderLogoTextSpan">Academium</span>
                <sup class="HeaderLogoTextPlus">+</sup>
            </a>
            <p class="FooterP">O Academium+ é um projeto que está sendo desenvolvido por alunos do 4° semestre do curso técnico de informática do CEAP, contendo cursos de diversas plataformas fazendo com que você tenha mais facilidade para achar cursos</p>
            <ul class="Sociais">
                <li><a href="#"><i class="fa fa-facebook"></i></a></li>
                <li><a href="#"><i class="fa fa-twitter"></i></a></li>
                <li><a href="#"><i class="fa fa-google-plus"></i></a></li>
                <li><a href="#"><i class="fa fa-youtube"></i></a></li>
                <li><a href="#"><i class="fa fa-linkedin-square"></i></a></li>
            </ul>
        </div>
        <div class="footer-bottom">
            <p>&copy;2020 Copyright: Academiumplus.com </p>
        </div>
    </footer>
</body>
</html>